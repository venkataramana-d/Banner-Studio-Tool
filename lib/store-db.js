...
async function insertSeed(sql) {
  const { offers, coupons, events } = seedData();
...
export async function getEvents() {
  const sql = await q();
  const rows = await sql`SELECT id, offer_id, date, country, placeholder, impressions, clicks, redemptions FROM events`;
  return rows.map((r) => ({
    id: r.id, offerId: r.offer_id,
    date: typeof r.date === "string" ? r.date.slice(0, 10) : new Date(r.date).toISOString().slice(0, 10),
    country: r.country, placeholder: r.placeholder,
...
export async function recordEvent(input) {
  ...
  await sql`INSERT INTO events (id, offer_id, date, country, placeholder, impressions, clicks, redemptions)
    VALUES (${id}, ${input.offerId}, ${date}, ${country}, ${input.placeholder || null}, ${i}, ${c}, ${r})

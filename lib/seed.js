...
export const eventId = (offerId, date, country, variant) => `ev_${offerId}_${date}_${country}${variant ? `_${variant}` : ""}`;
  ...
  for (const o of offers) {
    ...
    for (let d = 0; d < DAYS; d++) {
      ...
      countries.forEach((c, ci) => {
        ...
        rows.push({ id: eventId(o.id, date, c), offerId: o.id, date, country: c, placeholder: o.placeholder, variant: null, impressions, clicks, redemptions });
      });
    }
  }
  return rows;

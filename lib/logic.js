...
export function findConflicts(offer, offers) {
  // Only offers that actually serve (or are queued to) can conflict; a draft,
  // pending, rejected, paused or expired offer holds no slot.
  return offers.filter((o) => (o.status === "scheduled" || o.status === "live") && offersOverlap(offer, o));
}
...
export function marginOk(price, festPrice, floorPct = POLICY.marginFloorPct) {
  return festPrice >= price * (1 - floorPct / 100);
}

// ---- Blackout periods ----
// A blackout covers a day when its [startDate, endDate] (YYYY-MM-DD) span includes it.
// Lexicographic string compare is correct for that format.
export function blackoutFor(dateIso, blackouts = []) {
  if (!dateIso) return null;
  const d = String(dateIso).slice(0, 10);
  return blackouts.find((b) => b.startDate && b.endDate && b.startDate <= d && d <= b.endDate) || null;
}
// Any blackout that overlaps an offer's [startsAt, endsAt] window.
export function windowBlackout(startsAt, endsAt, blackouts = []) {
  if (!startsAt || !endsAt) return null;
  const s = String(startsAt).slice(0, 10), e = String(endsAt).slice(0, 10);
  return blackouts.find((b) => b.startDate && b.endDate && s <= b.endDate && b.startDate <= e) || null;
}
...
export const REASON_TEXT = {
  ...
  blackout: "Promotions are paused right now.",
};

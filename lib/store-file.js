...
export function updateOffer(id, patch) {
  ...
  if (i < 0) return null;
  const { id: _i, createdAt: _c, ...safe } = patch || {}; // never let a patch rewrite identity
  data.offers[i] = { ...data.offers[i], ...safe };
...
export function deleteOffer(id) {
  ...
  data.events = data.events.filter((e) => e.offerId !== id);
  write(data);
  return true;
}
...
export function recordEvent(input) {
  ...
  if (!row) {
    row = { id, offerId: input.offerId, date, country, placeholder: input.placeholder || null, variant, impressions: 0, clicks: 0, redemptions: 0 };
...
export function getBlackouts() {
  const { blackouts } = read();
  return [...blackouts].sort((a, b) => (a.startDate < b.startDate ? -1 : 1));
}
export function createBlackout(input) {
  const data = read();
  const bo = buildBlackout(input);
  data.blackouts.push(bo);
  write(data);
  return bo;
}
export function deleteBlackout(id) {
  const data = read();
  data.blackouts = data.blackouts.filter((b) => b.id !== id);
  write(data);
  return true;
}

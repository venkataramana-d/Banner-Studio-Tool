...
export function localize(text, region, festKey) {
  ...
  let out = text;
  const greeting = GREETINGS[festKey];
  if (greeting) out = `${greeting}! ${out.charAt(0).toUpperCase()}${out.slice(1)}`;
...
export function buildKit(festKey, courseId, opts = {}) {
  ...
  const headline = (greeting ? `${greeting}! ${fest.motivation}` : fest.motivation).slice(0, 90);
